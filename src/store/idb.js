import config from './idb.config.js'
    ...
    dbRequest.onupgradeneeded = (event) => {
      ...
      for (let i = 0; i < config.objectStores.length; i++) {
        ...
        if (!_db.objectStoreNames.contains(object.objectStoreName)) {
          const objectStore = _db.createObjectStore(object.objectStoreName, { keyPath: '_id', autoIncrement: true }) /* 创建person仓库(表) 主键 */
      ...
      if (typeof findInfo.indexKind !== 'undefined') {
        ...
        switch (findInfo.indexKind) {
          ...
          case '<=':
            ...
            break
          case 'between':
            var betweenInfo = findInfo.betweenInfo
